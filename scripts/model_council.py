...
class Council:
    ...
    def run_round(self, prompt: str) -> dict[str, str]:
        ...
        for p in self.participants:
            messages_map[p.name] = self._build_messages(p, prompt)
    ...
    def load_transcript(cls, path: str, api_key: str | None = None) -> Council:
        ...
        participants = [
            Participant(name=pd["name"], model=pd["model"], persona=pd["persona"])
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    new_parser.add_argument(
        ...
        required=True,
        nargs="+",
        help="Model IDs (e.g. deepseek/deepseek-v4-pro)",
    )
